# Copy audit

## Cold landing screen

| Copy | Words | Result |
| --- | ---: | --- |
| Appointment release board | 3 | Pass |
| Release dates, finalized 8 weeks out. | 6 | Pass |
| We open the appointment calendar 8 weeks in advance and lock dates two weeks before the event. | 17 | Pass |
| Review availability | 2 | Pass |
| Try it with sample data | 5 | Pass |
| Guests need no account | 4 | Pass |
| Owners approve every request | 4 | Pass |
| No tracking cookies | 3 | Pass |

## Remaining landing page

| Copy | Words | Result |
| --- | ---: | --- |
| Earliest start: 8 weeks | 4 | Pass |
| Working days: Mon—Thu | 3 | Pass |
| Pause dates: Holidays | 3 | Pass |
| Dates lock two weeks before each appointment. | 7 | Pass |
| The owner confirms the time or suggests another one before then. | 11 | Pass |
| Four weekday dates move together from open request to owner approval. | 11 | Pass |
| Guests see one clear status for every requested time. | 9 | Pass |
| Select a date to hear its current sample status. | 9 | Pass |
| The guest chooses an available time and shares only needed contact details. | 11 | Pass |
| The owner approves the request before the appointment becomes ready to confirm. | 12 | Pass |
| A private link keeps confirmation, changes, calendar export, and cancellation together. | 11 | Pass |
| Demo data stays in this browser. | 6 | Pass |
| Real desks store only booking details and consent. | 8 | Pass |
| This is not a payment system, staff rota, CRM, or automatic message sender. | 13 | Pass |
| The free desk holds 30 active bookings. | 7 | Pass |
| Panel Pro is a $29 one-time license. | 7 | Pass |
| Opens a ready-to-confirm sample. | 5 | Pass |
| Nothing is saved to a real desk. | 7 | Pass |

No sentence exceeds 22 words. No banned plain-words terms appear.

## Terminology

| Concept | Product term |
| --- | --- |
| A visitor booking an appointment | guest |
| A request that needs a decision | time request |
| A calendar date shown on the landing page | release date |
| The business operator | owner |
| The private status URL | private booking link |
| The isolated try-out | demo |
