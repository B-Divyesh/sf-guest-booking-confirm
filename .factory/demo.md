...
Open `/demo` or use **Try it with sample data** on the first screen. The demo starts with Maya Chen's realistic Northstar Barber appointment already approved, so the guest confirmation step is ready to try immediately. It safely demonstrates confirmation, a calendar download, another-time request, cancellation, and the owner's manual reminder checklist.
